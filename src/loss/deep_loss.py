...
class PNetLin(nn.Module):
    def __init__(self, pnet_type='vgg', pnet_rand=False, pnet_tune=False, use_dropout=True, use_gpu=True, spatial=False, version='0.1', colorspace='RGB', reduction='none'):
        ...
        if (self.pnet_tune):
            self.net = net_type(
                pretrained=not self.pnet_rand, requires_grad=True)
        else:
            self.net = [
        ...
        if (use_gpu):
            if (self.pnet_tune):
                ...
                self.net[0].cuda(0)
            self.shift = self.shift.cuda(0)
            self.scale = self.scale.cuda(0)
            self.lin0.cuda(0)
            self.lin1.cuda(0)
            self.lin2.cuda(0)
            self.lin3.cuda(0)
    ...
    def forward(self, in0, in1):
        ...
        if self.spatial:
            lin_models = [self.lin0, self.lin1,
                          self.lin2, self.lin3, self.lin4]
            if (self.pnet_type == 'squeeze'):
