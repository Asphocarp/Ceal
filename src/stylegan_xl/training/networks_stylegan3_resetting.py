...
def reshape(
    x: torch.Tensor,  # [B, X] (ab_map) after selecting range for flat maps
    shape: list[int],  # shape of each elem, 4 dim, currently (out, in, 1, 1)
    rank: Optional[int],  # rank before reshaping, None for no LoRA
    ) -> torch.Tensor:  # [B, *shape]
    if rank is None:
        return x.reshape(x.shape[0], *shape)
    # lora, first B then A, get BA
    assert len(shape) == 4
    # print('rank', rank, 'shape', shape, 'x.shape', x.shape)  # debug
    assert x.shape[1] == rank*(shape[0] + shape[1]*shape[2]*shape[3])
    b_shape = (shape[0], rank)
    a_shape = (rank, shape[1]*shape[2]*shape[3])  # note: typical lora breaks kernel topology
    b_size = math.prod(b_shape)
    b = x[:, :b_size].view(-1, *b_shape)
    a = x[:, b_size:].view(-1, *a_shape)
    maps = (b @ a).view(-1, *shape)
    return maps
...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, flat_maps, noise_mode='random', force_fp32=False, update_emas=False):
        ...
        if OLD:
            ...
            x = filtered_lrelu.filtered_lrelu(
                x=x, fu=self.up_filter, fd=self.down_filter, b=self.bias.to(x.dtype),
                up=self.up_factor, down=self.down_factor, padding=self.padding, gain=gain, slope=slope, clamp=self.conv_clamp)
        else:
            assert self.granularity in ['filter', 'kernel', 'float']
            assert self.rank is not None
            assert self.partition is not None
            # get wmaps (for weights, not style vector)
            ab_maps = flat_maps[:, self.partition[0]:self.partition[1]]
            wmap_shape = [self.out_channels, 1, 1, 1] if self.granularity == 'filter' \
                else [self.out_channels, self.in_channels, 1, 1] if self.granularity == 'kernel' \
                else [self.out_channels, self.in_channels, self.conv_kernel, self.conv_kernel] # float
            wmaps = reshape(ab_maps, wmap_shape, self.rank)
            # get bmaps (for bias)
            mod_bias = len(self.partition) > 2
            if mod_bias:
                b_flat_maps = flat_maps[:, self.partition[1]:self.partition[2]]
                bmaps = b_flat_maps.reshape([-1, self.out_channels])
            else:
                bmaps = [1 for _ in range(x.shape[0])]
            # per elem
            new_x = []
            for i, (x_i, style_i, wm_i, bm_i) in enumerate(zip(x.to(dtype), styles, wmaps, bmaps)):
                modulated_bias = self.bias * (1 + 0.01 * bm_i)
                modulated_weight = self.weight * (1 + 0.01 * wm_i)

                x_i = modulated_conv2d(
                    x=x_i.unsqueeze(0).to(dtype), w=modulated_weight, s=style_i.unsqueeze(0),
                ...
                x_i = filtered_lrelu.filtered_lrelu(
                    x=x_i, fu=self.up_filter, fd=self.down_filter,
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, flat_maps, **layer_kwargs):
        ...
        for name, w in zip(self.layer_names, ws[1:]):
            x = getattr(self, name)(x, w, flat_maps, **layer_kwargs)
...
class Generator(torch.nn.Module):
    ...
    def forward(self, z, c, flat_maps, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ws = self.mapping(z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
        img = self.synthesis(ws, flat_maps, update_emas=update_emas, **synthesis_kwargs)
        return img
