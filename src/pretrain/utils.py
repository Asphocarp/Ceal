...
def get_dataloader(data_dir, transform=utils_img.default_transform, batch_size=128, shuffle=True, num_workers=8, custom=True):
    ...
    dataset = ImageFolder(data_dir, transform=transform) if custom else datasets.ImageFolder(
...
def build_optimizer(name, model_params, **optim_params):
    ...
    raise ValueError(
        f'Unknown optimizer "{name}", choose among {str(tim_optimizers+torch_optimizers)}')
...
def build_lr_scheduler(name, optimizer, **lr_scheduler_params):
    ...
    raise ValueError(
        f'Unknown scheduler "{name}", choose among {str(tim_schedulers+torch_schedulers)}')
...
def init_distributed_mode(params):
    ...
    # multi-GPU job (local or multi-node) - jobs started with torch.distributed.launch
        ...
        os.environ['MASTER_ADDR'] = os.environ['MASTER']
...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        print('{} Total time: {} ({:.6f} s / it)'.format(header,
              total_time_str, total_time / (len(iterable)+1)))
