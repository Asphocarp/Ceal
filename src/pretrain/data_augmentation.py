...
class RandomDiffJPEG(AugmentationBase2D):
    def __init__(self, p, low=10, high=100) -> None:
        super().__init__(p=p)
        self.diff_jpegs = [DiffJPEG(quality=qf).to(device)
                           for qf in range(low, high, 10)]
...
class RandomBlur(AugmentationBase2D):
    def __init__(self, blur_size, p=1) -> None:
        super().__init__(p=p)
        self.gaussian_blurs = [K.RandomGaussianBlur(kernel_size=(kk, kk), sigma=(
            kk*0.15 + 0.35, kk*0.15 + 0.35)) for kk in range(1, int(blur_size), 2)]

    def generate_parameters(self, input_shape: torch.Size):
        blur_strength = torch.randint(
            high=len(self.gaussian_blurs), size=input_shape[0:1])
...
class HiddenAug(nn.Module):
    ...
    def __init__(self, img_size, p_crop=0.3, p_blur=0.3, p_jpeg=0.3, p_rot=0.3, p_color_jitter=0.3, p_res=0.3):
        ...
        if p_res > 0:
            ...
            res1 = K.RandomResizedCrop(
                size=(res1, res1), scale=(1.0, 1.0), p=1)  # Resize 0.3
            res2 = K.RandomResizedCrop(
                size=(res2, res2), scale=(1.0, 1.0), p=1)  # Resize 0.7
            augmentations += [res1, res2]
        if p_blur > 0:
            blur1 = K.RandomGaussianBlur(kernel_size=(
                11, 11), sigma=(2.0, 2.0), p=1)  # Gaussian blur σ = 2
        ...
        if p_color_jitter > 0:
            jitter1 = K.ColorJiggle(brightness=(
                1.5, 1.5), contrast=0, saturation=0, hue=0, p=1)
            jitter2 = K.ColorJiggle(brightness=0, contrast=(
                1.5, 1.5), saturation=0, hue=0, p=1)
            jitter3 = K.ColorJiggle(
                brightness=0, contrast=0, saturation=(1.5, 1.5), hue=0, p=1)
            jitter4 = K.ColorJiggle(
                brightness=0, contrast=0, saturation=0, hue=(0.25, 0.25), p=1)
            augmentations += [jitter1, jitter2, jitter3, jitter4]
        self.hidden_aug = K.AugmentationSequential(
            *augmentations, random_apply=1).to(device)
...
class KorniaAug(nn.Module):
    def __init__(self, degrees=30, crop_scale=(0.2, 1.0), crop_ratio=(3/4, 4/3), blur_size=17, color_jitter=(1.0, 1.0, 1.0, 0.3), diff_jpeg=10,
        ...
        self.crop = K.RandomResizedCrop(size=(img_size, img_size), scale=crop_scale,
                                        ratio=crop_ratio, p=p_crop, cropping_mode=cropping_mode).to(device)
        self.hflip = K.RandomHorizontalFlip().to(device)
        self.blur = RandomBlur(blur_size, p_blur).to(device)
        self.diff_jpeg = RandomDiffJPEG(
            p=p_diff_jpeg, low=diff_jpeg).to(device)
