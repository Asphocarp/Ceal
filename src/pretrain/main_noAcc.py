...
def main(params: argparse.Namespace):
    ...
    if params.decoder == 'hidden':
        ...
        raise ValueError('')
    elif params.decoder == 'clip':  # NOW
        # huggingface-cli download openai/clip-vit-base-patch32
        hiddec = models.HidDec(hids=13, width=768, nbit=params.num_bits, device=device)
    ...
    lr_mult = params.batch_size * WORLD_SIZE / 512.0
    ...
    print('total parameters: %d'%sum(p.numel() for p in to_optim))
    ...
    for epoch in range(start_epoch, params.epochs):
        ...
        if True:
            ...
            torch.save(save_dict, os.path.join(params.output_dir, 'checkpoint.pth'))
            if params.saveckp_freq and epoch % params.saveckp_freq == 0:
                torch.save(save_dict, os.path.join(params.output_dir, f'checkpoint{epoch:03}.pth'))
