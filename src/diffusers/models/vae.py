...
from ..utils.constants import USE_MAPPER
from .lora import ConvWithMapper
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.conv_act = nn.SiLU()
        if USE_MAPPER:
            self.conv_out = ConvWithMapper(block_out_channels[0], out_channels, 3, padding=1)
        else:
    ...
    def forward(
        ...
        if self.training and self.gradient_checkpointing:
            ...
            if is_torch_version(">=", "1.11.0"):
                ...
                # middle
                sample = torch.utils.checkpoint.checkpoint(
                    create_custom_forward(self.mid_block), sample, latent_embeds, maps=maps
                )
                sample = sample.to(upscale_dtype)

                # up
                for up_block in self.up_blocks:
                    sample = torch.utils.checkpoint.checkpoint(create_custom_forward(up_block), sample, latent_embeds, maps=maps)
        else:
            # middle
