...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        if not with_fp:
            ...
            # > new -------------------------------------
            # args = () if USE_PEFT_BACKEND else (scale,) # not considering LoRA for now

            phis_q = attn.affine_q(encoded_fingerprint)
            query = torch.bmm(hidden_states, phis_q.unsqueeze(-1) * attn.to_q.weight.t().unsqueeze(0)) + attn.to_q.bias
            ...
            phis_k = attn.affine_k(encoded_fingerprint)
            key = torch.bmm(encoder_hidden_states, phis_k.unsqueeze(-1) * attn.to_k.weight.t().unsqueeze(0)) + attn.to_k.bias

            phis_v = attn.affine_v(encoded_fingerprint)
            value = torch.bmm(encoder_hidden_states, phis_v.unsqueeze(-1) * attn.to_v.weight.t().unsqueeze(0)) + attn.to_v.bias
            # > new -------------------------------------
