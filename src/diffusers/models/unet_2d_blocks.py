...
class UNetMidBlock2D(nn.Module):
    ...
    def forward(self,
        ...
        hidden_states = self.resnets[0](hidden_states, temb, maps=maps)
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            if attn is not None:
                hidden_states = attn(hidden_states, temb=temb)
            hidden_states = resnet(hidden_states, temb, maps=maps)
...
class UpDecoderBlock2D(nn.Module):
    ...
    def forward(
        ...
        for resnet in self.resnets:
            hidden_states = resnet(hidden_states, temb=temb, scale=scale, maps=maps)

        if self.upsamplers is not None:
            for upsampler in self.upsamplers:
                hidden_states = upsampler(hidden_states, maps=maps)
